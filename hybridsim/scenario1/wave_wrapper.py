...
def run_wave(mesh_input, max_freq, recx, recy, recz, source_pos, rir_duration,):
    ...
    result_filename = f"{mesh_used}_{max_freq}Hz_{rir_duration}s_S2R2"  # name of the result file. The result file will be saved in the same folder as this script. The result file will be saved in .mat format.
    ...
    for material, label in BC_labels.items():
        if material == "hard wall":
            BC_para.append({"label": label, "RI": 0.9487})
        else:
